    ...
    <div className={`${isNavOpen ? "left-0":"left-[100%]"} fixed bottom-0 top-0 z-20 flex h-screen w-[75%] flex-col justify-between bg-green-100 px-8 pb-6 pt-16 text-black md:hidden rounded-r-xl shadow-md transition-all`}>
      <div>
        ...
        <nav className='mt-12'>
            ...
            <Link to={"/"} className="cursor-pointer" onClick={()=>setIsNavOpen(false)}>
              <li className="hover:text-orange-400">Home</li>
            </Link>
            <Link to={"/shop"} className="cursor-pointer" onClick={()=>setIsNavOpen(false)}>
              <li className="hover:text-orange-400">Shop</li>
            </Link>
            <Link to={"/about"} className="cursor-pointer" onClick={()=>setIsNavOpen(false)}>
              <li className="hover:text-orange-400">About</li>
            </Link>
            <Link to={"/contact"} className="cursor-pointer" onClick={()=>setIsNavOpen(false)}>
              <li className="hover:text-orange-400">Contact</li>
            </Link>
            <Link>
                <button onClick={()=>setIsNavOpen(false)} className='px-3 py-1 w-max rounded-md bg-green-600 text-white flex items-center gap-1'>Login<FaChevronRight/></button>
            </Link>
            </ul>
        </nav>
      </div>
